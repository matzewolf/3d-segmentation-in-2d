...
for i_seg in range(label_max - label_min +1):
    idxs = np.where(pre_pt == i_seg)[0]
    color = current_palette.as_hex()[i_seg]
    fig = go.Figure(data=[go.Scatter3d(x=x_pt[idxs,0], y=x_pt[idxs,1], z=x_pt[idxs,2],
                                   mode='markers',
...
#%% initialization
result_path = 'ShapeNet_overfit_result.hdf5'
class_name = np.genfromtxt('all_object_categories.txt',dtype='U')[:,0]
#############################################################################
# ['Airplane' 'Bag' 'Cap' 'Car' 'Chair' 'Earphone' 'Guitar' 'Knife'         #
# 'Lamp' 'Laptop' 'Motorbike' 'Mug' 'Pistol' 'Rocket' 'Skateboard' 'Table'] #
...
idx_sample_list = np.where(y_test==4)[0]
idx_sample = idx_sample_list[0]
label_min = s_test.min()
label_max = s_test.max()
print('Class_name:',class_name[4],', test sample id:',idx_sample)
#%% load the test sample
x_pt = x_test[idx_sample]
s_pt = s_test[idx_sample]-label_min
pre_pt = pre_test[idx_sample]-label_min
