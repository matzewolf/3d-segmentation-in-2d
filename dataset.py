...
class ShapeNetPartDataset(Dataset):
    def __init__(self, path: Path = Path('hdf5_data'),
        ...
        """
        PyTorch dataset class for the Stanford ShapeNet Part dataset, however with some specific preprocessing steps,
        see `combine_dataset` and `prepare_dataset` as well as the paper.

        :param path: Path to the downloaded ShapeNet Part directory.
        :param split: Which data split to process, either 'train', 'val', 'test' or 'overfit'. Overfit dataset consists
            of the first instance in the val dataset and can be used for sanity checks on the train/val process.
        :param size_sub: tdb
        """
        self.num_classes = 50
        ...
        if split != 'overfit':
            ...
            points_3d = torch.from_numpy(self.data[f"x_val"])
            parts = torch.from_numpy(self.data[f"s_val"])
            points_2d = torch.from_numpy(self.data[f"p_val"])
    ...
    def __getitem__(self, item) -> tuple[Tensor, Tensor]:
        ...
        :return: Tuple of two tensors. First tensor contains an image with 3 channels representing the 3D coordinates of
            the corresponding 3D points. Second tensor contains the image with 1 channel representing the part label,
            which is NOT one-hot encoded.
        """
